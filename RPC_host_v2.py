...
class RPCHost(object):
    ...
    def call(self, rpcMethod, *params):
        payload=None
        if not isinstance(rpcMethod, list)  :
            payload = json.dumps({"method": rpcMethod, "params": list(params), "jsonrpc": "2.0"})
        else:
            request_list=[]
            
            for i in range(0,len(rpcMethod)):
                request_list.append({"method": rpcMethod[i], "params": list(params[0][i]), "jsonrpc": "2.0"})
            payload = json.dumps(request_list)
